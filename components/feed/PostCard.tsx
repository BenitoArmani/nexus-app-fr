...
import { Heart, MessageCircle, Share2, Bookmark, MoreHorizontal, Eye, Lock, Flag, Copy } from 'lucide-react'
...
export default function PostCard({ post, onLike }: PostCardProps) {
  ...
  const handleReport = () => {
    toast('Signalement envoyé. Merci.', { icon: '🛡️', style: { background: '#1a0a2e', color: '#e2e8f0', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '12px' } })
    setShowMenu(false)
  }
    ...
    <>
      ...
      >
        ...
        <div className="flex items-start justify-between p-5 pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-text-muted">{post.user?.username ? `@${post.user.username}` : ''} · {timeAgo(post.created_at)}</p>
            </div>
          </div>

          <div className="relative" ref={menuRef}>
            <button onClick={() => setShowMenu(m => !m)} className="w-8 h-8 rounded-xl hover:bg-white/5 flex items-center justify-center text-text-muted hover:text-text-primary transition-colors">
            ...
            <AnimatePresence>
              {showMenu && (
                ...
                >
                  ...
                  <button onClick={() => { toast.success('Partagé !', { style: { background: '#1a0a2e', color: '#c084fc', border: '1px solid rgba(139,92,246,0.3)', borderRadius: '12px' } }); setShowMenu(false) }} className="w-full flex items-center gap-2.5 px-4 py-3 text-sm text-zinc-200 hover:bg-white/5 transition-colors">
        ...
        <div className="flex items-center gap-4 px-5 py-2 text-sm text-text-muted border-t border-white/5">
          <span className="flex items-center gap-1"><Eye size={12} /> {formatNumber(post.views)} vues</span>
          <span>{formatNumber(post.likes_count)} j&apos;aime</span>
          <button onClick={() => setShowComments(true)} className="hover:text-cyan-400 transition-colors">
            {formatNumber(post.comments_count)} commentaires
          </button>
        </div>
        ...
        <div className="flex items-center gap-1 px-2 py-2 border-t border-white/5">
          ...
          {post.user?.id && !post.bets_disabled && (
            <TipButton toUserId={post.user.id} toUsername={post.user.username ?? ''} context="post" contextId={post.id} />
          )}

          <motion.button
            whileTap={{ scale: 0.85 }}
            onClick={() => setSaved(!saved)}
            className={`flex items-center justify-center w-10 py-2 rounded-xl text-sm font-medium transition-colors ${saved ? 'text-amber-400' : 'text-text-muted hover:text-amber-400 hover:bg-amber-500/5'}`}
          >
